...
import m.vita.module.track.comp.LCD;
...
public class DreamPowerCalculator implements PhonePowerCalculator {
	...
	public double getCpuPower(CPU.CpuData data) {
		...
		if (powerRatios.length == 1) {
			...
			if (ind == freqs.length)
				ind--;
			ratio = powerRatios[ind - 1]
					+ (powerRatios[ind] - powerRatios[ind - 1])
					/ (freqs[ind] - freqs[ind - 1]) * (sfreq - freqs[ind - 1]);
		}
	...
	public double getWifiPower(Wifi.WifiData data) {
		if (!data.wifiOn) {
			...
			if (linkSpeeds.length == 1) {
				...
				/*
				 * Find the two nearest speed/ratio pairs and linearly
				 * interpolate the ratio for this link speed.
				 */
				...
				if (ind == linkSpeeds.length)
					ind--;
				ratio = linkRatios[ind - 1]
						+ (linkRatios[ind] - linkRatios[ind - 1])
						/ (linkSpeeds[ind] - linkSpeeds[ind - 1])
						* (data.linkSpeed - linkSpeeds[ind - 1]);
			}
			return Math
					.max(0, coeffs.wifiHighPower() + ratio * data.uplinkRate);
		}
