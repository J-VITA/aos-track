...
public class Sensors extends PowerComponent {
	private final String TAG = "NT1Sensors";
...
//		this.context = context;
		...
		if (!NotificationService.available()) {
			Log.w(TAG,
					"Sensor component created although no notification service "
	...
	private static class SensorStateKeeper {
		...
		public void setupSensorTimes(double[] sensorTimes,
			...
			for (int i = 0; i < MAX_SENSORS; i++) {
				sensorTimes[i] = 1.0
						* (times[i] + (nesting[i] > 0 ? now - lastTime : 0))
						/ div;
				times[i] = 0;
			}
			lastTime = now;
