...
public class Wifi extends PowerComponent {
	...
	private static final String TAG = "NT1Wifi";
	...
	public IterationData calculateIteration(long iteration) {
		...
		// Branch for Some Devices which is not supported with TrafficStats class
		if (totalTxPackets == TrafficStats.UNSUPPORTED || totalRxPackets == TrafficStats.UNSUPPORTED ||
				totalTxBytes == TrafficStats.UNSUPPORTED || totalRxBytes == TrafficStats.UNSUPPORTED) {
			transmitPackets = sysInfo.readLongFromFile(transPacketsFile);
		    receivePackets = sysInfo.readLongFromFile(readPacketsFile);
		    transmitBytes = sysInfo.readLongFromFile(transBytesFile);
		    receiveBytes = sysInfo.readLongFromFile(readBytesFile);
		} else {
			transmitPackets = totalTxPackets - TrafficStats.getMobileTxPackets();
			receivePackets = totalRxPackets - TrafficStats.getMobileRxPackets();
			transmitBytes = totalTxBytes - TrafficStats.getMobileTxBytes();
			receiveBytes = totalRxBytes - TrafficStats.getMobileRxBytes();
		}
		 ...
		 */
		if (iteration % 1 == 0 || lastLinkSpeed == -1) {
		...
		if (wifiState.isInitialized()) {
			...
			data.init(wifiState.getPackets(), wifiState.getUplinkBytes(),
					wifiState.getDownlinkBytes(), wifiState.getUplinkRate(), linkSpeed,
					wifiState.getPowerState());
			result.setPowerData(data);
		} else {
			wifiState.updateState(transmitPackets, receivePackets, transmitBytes, receiveBytes);
		}
		...
		/* 
		 * TODO : Android File System has changed.
		 *  	Since JellyBean MR2, There's no "/proc/uid_stat" directory.
		 *  	So We have to make branch, 'Before JB_MR2' and else.  
		 */
		// File System has changed since 4.3
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR2) {
			lastUids = sysInfo.getUids(lastUids);
		} else {
			lastUids = TrafficUids.getUids(lastUids);
		}
		...
		if (lastUids != null) {
			for (int uid : lastUids) {
				...
				try {
					...
					// Branch For Before JellyBeanMR2
					if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR2) {
						...
						transmitBytes = TrafficStats.getUidTxBytes(uid);
					}
