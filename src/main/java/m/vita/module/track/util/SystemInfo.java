...
public class SystemInfo {
    private static final String TAG = Globals.LOG_TAG + " SystemInfo";
    ...
    public static final int AID_ALL = -1; /* A special constant we will use to
     * indicate a request for global information.
     */
    ...
    private static final int[] READ_LONG_FORMAT = new int[] { PROC_SPACE_TERM
            | PROC_OUT_LONG };
    ...
    private static final int[] PROCESS_TOTAL_STATS_FORMAT = new int[] {
            PROC_SPACE_TERM, PROC_SPACE_TERM | PROC_OUT_LONG,
            PROC_SPACE_TERM | PROC_OUT_LONG, PROC_SPACE_TERM | PROC_OUT_LONG,
            PROC_SPACE_TERM | PROC_OUT_LONG, PROC_SPACE_TERM | PROC_OUT_LONG,
            PROC_SPACE_TERM | PROC_OUT_LONG, PROC_SPACE_TERM | PROC_OUT_LONG, };
    private static final int[] PROC_MEMINFO_FORMAT = new int[] {
            PROC_SPACE_TERM | PROC_COMBINE, PROC_SPACE_TERM | PROC_OUT_LONG,
            PROC_LINE_TERM, PROC_SPACE_TERM | PROC_COMBINE,
            PROC_SPACE_TERM | PROC_OUT_LONG, PROC_LINE_TERM,
            PROC_SPACE_TERM | PROC_COMBINE, PROC_SPACE_TERM | PROC_OUT_LONG,
            PROC_LINE_TERM, PROC_SPACE_TERM | PROC_COMBINE,
            PROC_SPACE_TERM | PROC_OUT_LONG, PROC_LINE_TERM, };
    ...
    private SystemInfo() {
        ...
        try {
            methodReadProcFile = Process.class.getMethod("readProcFile",
            ...
            try {
                ...
                for (String line = rdr.readLine(); line != null; line = rdr
                    ...
                    if (line.startsWith("Uid:")) {
                        ...
                        String realUidToken = tokens[tokens[0].length() == 0 ? 1
                                : 0];
                        try {
    ...
    public boolean getPidUsrSysTime(int pid, long[] times) {
        if (methodReadProcFile == null)
            return false;
        try {
            return (Boolean) methodReadProcFile.invoke(null, "/proc/" + pid
                    + "/stat", PROCESS_STATS_FORMAT, null, times, null);
        } catch (IllegalAccessException e) {
    ...
    /*
     * times should contain seven elements. times[INDEX_USER_TIME] will be
     * filled with the total user time, times[INDEX_SYS_TIME] will be filled
    ...
    public boolean getUsrSysTotalTime(long[] times) {
        if (methodReadProcFile == null)
            return false;
        try {
            if ((Boolean) methodReadProcFile.invoke(null, "/proc/stat",
     ...
     * in kb, and mem[INDEX_MEM_CACHED] will give the size of kernel caches in
     * kb. Returns true on success.
     */
    public boolean getMemInfo(long[] mem) {
        if (methodReadProcFile == null)
            return false;
        try {
            if ((Boolean) methodReadProcFile.invoke(null, "/proc/meminfo",
    ...
    public long readLongFromFile(String file) {
        if (methodReadProcFile == null)
            return -1;
        try {
            if ((Boolean) methodReadProcFile.invoke(null, file,
                    READ_LONG_FORMAT, null, readBuf, null)) {
                return readBuf[0];
            }
        } catch (IllegalAccessException e) {
            Log.w(TAG, "Failed to get pid cpu usage");
